...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        int index2 = parentIndex(index);
        if (contents[index2].myPriority > contents[index].myPriority) {
            swap(index2, index);
            swim(index2);
        }
        return;
    ...
    private void sink(int index) {
        ...
        if (index == size || contents[leftIndex(index)] == null) {
            return;
        }
        int index2 = leftIndex(index);
        int index3;
        if (contents[rightIndex(index)] == null) {
            index3 = -1;
        } else {
            index3 = rightIndex(index);
        }
        double p1 = contents[index].myPriority;
        double p2 = contents[index2].myPriority;
